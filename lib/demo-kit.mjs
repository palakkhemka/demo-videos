...
export function makeSocialCuts(mp4) {
  ...
  for (const [name, W, H] of targets) {
    ...
    // bg = same video zoomed to COVER the frame + blurred; fg = video scaled to
    // fit (fills the width), centered over the blurred bg.
    const filter =
      ...
      `[bg][fg]overlay=(W-w)/2:(H-h)/2:shortest=1,format=yuv420p[out]`
    const r = run(['-i', mp4, '-filter_complex', filter, '-map', '[out]', '-c:v', 'libx264', '-movflags', '+faststart', out])
