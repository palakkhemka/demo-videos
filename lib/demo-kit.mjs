...
export async function recordTool(config) {
  ...
  const cardOpts = {
    ...
    bgImage: env('CARD_BG_IMAGE', '') ? path.resolve(DEMO_DIR, env('CARD_BG_IMAGE')) : '',
    bgColor: env('CARD_BG_COLOR', '14294a'),
    showLogo: env('CARD_LOGO', '1') !== '0',
    W: OW, H: OH,
  }
  console.log('[fonts]', FONT_PATHS.ui, FONT_PATHS.uib)
  renderCard({ out: intro, dur: 3, title: 'Textile Designer AI', subtitle: displayName, ...cardOpts })
  renderCard({ out: outro, dur: 3.6, title: 'Visit textile-designer.ai', subtitle: 'AI tools for textile & fashion design', ...cardOpts })
