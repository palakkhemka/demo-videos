...
export async function recordTool(config) {
  const {
    ...
    outName = `${tabId}-demo`,
    selectModel1 = false, // bg_remove has a Model 1/2 toggle
  ...
  const dir = path.join(OUT, outName.replace(/-demo$/, '')) // per-tool folder
  ...
  try {
    ...
    await page.screenshot({ path: path.join(dir, `${outName}-final.png`) }).catch(() => {})
  ...
  const base = suffix ? `${outName}-${suffix}` : outName
  const webms = fs
    .readdirSync(dir)
  ...
  // 2) Title + end cards.
  const card = (out, dur, title, subtitle, titleSize) => run([
    '-f', 'lavfi', '-i', `gradients=s=${OW}x${OH}:c0=0x14294a:c1=0x2f6a4a:d=${dur}`,
    '-loop', '1', '-t', String(dur), '-i', logo,
    '-filter_complex',
    `[1:v]scale=300:300[lg];[0:v][lg]overlay=(W-w)/2:(H-h)/2-150[bg];` +
    `[bg]drawtext=fontfile=${uib}:text='${title}':fontcolor=white:fontsize=${titleSize}:x=(w-text_w)/2:y=H/2+90,` +
    `drawtext=fontfile=${ui}:text='${subtitle}':fontcolor=0xbcd6c8:fontsize=36:x=(w-text_w)/2:y=H/2+185,` +
    `fade=t=in:st=0:d=0.5,fade=t=out:st=${dur - 0.5}:d=0.5,format=yuv420p[out]`,
    '-map', '[out]', '-r', '25', '-c:v', 'libx264', out,
  ])
  card(intro, 3, 'Textile Designer AI', displayName, 70)
  card(outro, 3.6, 'Visit textile-designer.ai', 'AI tools for textile & fashion design', 64)
...
export async function createDemoSession({ dirName, acceptDownloads = true }) {
  const OUT = env('OUT', path.join(DEMO_DIR, 'output'))
  ...
  page.on('download', async (d) => {
    const fp = path.join(dlDir, d.suggestedFilename() || `dl-${downloads.length}.png`)
    await d.saveAs(fp).catch(() => {})
    downloads.push(fp)
  })

  return { ctx, page, glide, fakeUpload, zoomEl, downloads, dir, t0, viewport: KIT_VIEWPORT }
}
...
// speed-ramp + intro/outro cards -> `${outName}.mp4`.
export async function finalizeVideo({
  ...
  const FFMPEG = path.join(DEMO_DIR, 'node_modules', 'ffmpeg-static', 'ffmpeg.exe')
  const suffix = env('DEMO_SUFFIX', '')
  const baseName = suffix ? `${outName}-${suffix}` : outName
  ...
  if (r.status !== 0) return console.log('frame failed:\n', (r.stderr || '').split('\n').slice(-8).join('\n'))
  const card = (out, dur, title, subtitle, titleSize) => run(['-f', 'lavfi', '-i', `gradients=s=${OW}x${OH}:c0=0x14294a:c1=0x2f6a4a:d=${dur}`, '-loop', '1', '-t', String(dur), '-i', logo, '-filter_complex', `[1:v]scale=300:300[lg];[0:v][lg]overlay=(W-w)/2:(H-h)/2-150[bg];[bg]drawtext=fontfile=${uib}:text='${title}':fontcolor=white:fontsize=${titleSize}:x=(w-text_w)/2:y=H/2+90,drawtext=fontfile=${ui}:text='${subtitle}':fontcolor=0xbcd6c8:fontsize=36:x=(w-text_w)/2:y=H/2+185,fade=t=in:st=0:d=0.5,fade=t=out:st=${dur - 0.5}:d=0.5,format=yuv420p[out]`, '-map', '[out]', '-r', '25', '-c:v', 'libx264', out])
  card(intro, 3, 'Textile Designer AI', displayName, 70)
  card(outro, 3.6, 'Visit textile-designer.ai', 'AI tools for textile & fashion design', 64)
...
// Produce social aspect-ratio variants of a finished mp4 (scaled-to-fit on the
// brand gradient): 9:16 (Reels/Shorts/TikTok), 1:1 (IG/LinkedIn feed),
// 16:9 (YouTube/LinkedIn/landing). Writes <base>-9x16/-1x1/-16x9.mp4.
export function makeSocialCuts(mp4) {
  ...
  const targets = [['9x16', 1080, 1920], ['1x1', 1080, 1080], ['16x9', 1920, 1080]]
  for (const [name, W, H] of targets) {
    const out = path.join(dir, `${b}-${name}.mp4`)
    const filter = `[0:v]scale=${W}:${H}:force_original_aspect_ratio=decrease[v];[1:v][v]overlay=(W-w)/2:(H-h)/2:shortest=1,format=yuv420p[out]`
    const r = run(['-i', mp4, '-f', 'lavfi', '-i', `gradients=s=${W}x${H}:c0=0x1f3a5f:c1=0x3f7d54:d=600`,
      '-filter_complex', filter, '-map', '[out]', '-c:v', 'libx264', '-movflags', '+faststart', out])
    console.log(r.status === 0 ? `  variant: ${path.basename(out)}` : `  variant ${name} failed`)
  }
}
