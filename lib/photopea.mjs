...
export async function clickColorLayers(page, { sleep, max = 8, prefix = 'color_', onBanner } = {}) {
  ...
  const toToggle = picks.slice(0, max)
  let toggled = 0
  // Toggle each layer's visibility (the eye button): hide -> short pause so the
  // image visibly loses that color -> show again. Driving `layer.visible` via
  // script flips the eye icon in the Layers panel AND removes the layer's
  // contribution to the composite, so it reads as a real eye-click on camera.
  for (let k = 0; k < toToggle.length; k++) {
    const { i, name } = toToggle[k]
    await show(onBanner, `Layer ${k + 1} of ${toToggle.length}`, '#2f7d54', `${name} - eye off`)
    ...
    await sleep(900)
    await show(onBanner, `Layer ${k + 1} of ${toToggle.length}`, '#2f7d54', `${name} - eye on`)
    const on = await runPhotopeaScript(page, `
      var L = app.activeDocument.layers[${i}];
      L.visible = true;
      __ret = { name: L.name, visible: L.visible };
    `, { timeoutMs: 3000 })
    if (!on.ok || !on.value || on.value.visible !== true) {
      console.log('[photopea] eye on failed for', name, on.error || on.value)
      continue
    }
    toggled++
    console.log('[photopea] toggled eye on', name)
    await sleep(700)
  }
  if (!toggled) console.log('[photopea] WARNING: no layers toggled')
  return toggled
}
